"""Scoring a detector against ground truth.

Anomalies can be counted two ways, and the two answers are both legitimate:

**Point-based.** Every labelled timestamp is one item, and the usual
confusion-matrix arithmetic applies. Useful when the cost of an anomaly scales
with how long it lasts.

**Event-based.** Every anomalous *interval* is one item, however long it is. A
true event counts as detected when at least ``thresh`` of its duration is
covered by the prediction, so an alert that fires part-way through a six-hour
outage is a hit rather than 40% of one. Useful when the cost is per incident —
one page, one investigation — which is usually what an operator cares about.

Coverage is measured in nanoseconds, and :attr:`~hazure.events.Events.durations`
counts inclusive bounds as the half-open span they occupy. An event spanning *k*
samples of a regular series therefore lasts exactly *k* steps, so ``thresh`` can
be read as a fraction of the event's samples and needs no separate rule for an
instantaneous event — such an event lasts 1 ns and is either covered or not.

Each function here picks the mode from what it is handed: label series give
point-based scores, :class:`~hazure.events.Events` (or plain lists of intervals)
give event-based ones, and a dict of either gives a dict of scores. Every option
propagates unchanged through the dict and multi-column paths.

A score that is not defined — recall with no true events, IoU over an empty
union — comes back as ``float("nan")`` rather than raising, so a sweep over many
configurations does not stop at the first degenerate one.
"""
...
__all__ = ["f1_score", "iou", "precision", "recall"]
