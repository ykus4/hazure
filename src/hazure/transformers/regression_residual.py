...
from hazure._core import Transformer
...
class RegressionResidual(Transformer):
    ...
    regressor
        ...
        numpy arrays. Defaults to :class:`OrdinaryLeastSquares`. It is copied at
        fit time, so the object passed stays unfitted and can be shared.

    Attributes
    ----------
    regressor_ : Regressor
        The fitted regressor: a copy of ``regressor`` when one was given.
    ...
    def _learn(self, ts: TimeSeries) -> None:
        ...
        self.regressor_ = (
            ...
            else copy.deepcopy(self.regressor)
        )
        self.regressor_.fit(
