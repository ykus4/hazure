...
class CustomizedTransformer(Transformer):
    ...
    * ``fit_func(values, **fit_func_params)`` receives the same kind of array
    ...
    def is_trainable(self) -> bool:
        """True when there is a ``fit_func`` to learn with.

        Without one nothing can be learned, so requiring fit() would be ceremony.
        """
        return self.fit_func is not None
