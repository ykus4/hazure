...
from hazure._core import Scorer
...
if TYPE_CHECKING:
    from hazure._core import TimeSeries, Transformer
...
class AutoregressionResidualScorer(Scorer):
    ...
    Attributes
    ----------
    transformer_ : hazure.transformers.RegressionResidual
    ...
    _trained: bool = True
    _persisted: ClassVar[tuple[str, ...]] = (*Scorer._persisted, "_trained")
    ...
    def _learn(self, ts: TimeSeries) -> None:
        ...
        self.transformer_ = RegressionResidual(target="t-0", regressor=self.regressor)
