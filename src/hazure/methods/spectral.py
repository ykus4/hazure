"""Saliency in the frequency domain.

A series is transformed, the *predictable* part of its amplitude spectrum is
removed, and the remainder is transformed back. What survives is the part of the
signal the spectrum could not account for smoothly — the saliency map — and a
point whose saliency stands well above its neighbours is the anomaly.

The appeal of working in the frequency domain is that nothing has to be assumed
about shape. No period is supplied, no trend model is fitted, no distribution is
claimed: a repeating structure of any length shows up as concentrated amplitude
and is removed by the same averaging step that removes everything else smooth.

The transform is defined on a fixed, evenly spaced grid, so a regular time axis
is required.
"""
...
__all__ = ["SpectralResidualDetector", "SpectralResidualScorer"]
